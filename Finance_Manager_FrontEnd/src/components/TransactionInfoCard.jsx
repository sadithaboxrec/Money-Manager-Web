...
const TransactionInfoCard = ({icon, title, date, amount, type, hideDeleteBtn, onDelete}) => {
    ...
    return (
        <div className="group relative flex items-center gap-4 mt-2 p-3 rounded-lg border-2 border-emerald-100 hover:border-emerald-300 hover:bg-emerald-50/40">
            <div className="w-12 h-12 flex items-center justify-center text-xl text-gray-800 bg-gray-100 rounded-full">
                {icon ? (
                    <img src={icon} alt={title} className="w-6 h-6" />
                ): (
                    <UtensilsCrossed className="text-emerald-800" />
                )}
            </div>
